...
\textbf{Can be automated:} No. This is the core design challenge and currently requires human insight. Automated learning of restriction maps from data is an open problem (see the Discussion section of the main manuscript).
...
If $\sigma$ is unknown, use Procedure 1 (spectral gap estimation) in Section~\ref{sec:parameter_selection} to adaptively choose $\epsilon$.
...
\textbf{Action:} Use one of the procedures in Section~\ref{sec:parameter_selection}:
\begin{itemize}
...
\textbf{Action:} Run the STPGC algorithm (Algorithm~1 in the main manuscript) with the graph, stalks, restriction maps, and $\epsilon$ from Steps 1-5. The output is:
\begin{itemize}
...
\textbf{Key insight:} Steps 1-3 (sheaf design) require human expertise and are the main barrier to widespread adoption. Steps 4-6 (computation) are fully algorithmic. Future work on automated sheaf learning (see the Discussion section of the main manuscript) aims to reduce the expertise required for Steps 2-3.
