...
The theoretical bounds derived above allow us to predict the robustness of $\Phi$ in our grid-world Bellman consistency experiment (Section~\ref{sec:safety_gym}) without running a separate noise-injection study. In the $8\times 8$ grid MDP:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Choose $\epsilon$ Adaptively:} Use the spectral gap method (Section~\ref{sec:epsilon_procedure}) rather than a fixed threshold.
