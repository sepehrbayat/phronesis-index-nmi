...
The complexity claim in Theorem~\ref{thm:complexity} assumes the well-conditioned case, which holds for all scenarios tested in Section~\ref{sec:experiments}.
...
\textbf{Setup:} 10 robots navigate a warehouse, sharing position estimates. Each robot has a local coordinate frame (rotated relative to a global frame).
...
\begin{itemize}
    ...
    \item \textbf{Effective:} Improves safety in RL (23\% cost reduction vs. PPO baseline, $p < 0.01$).
...
The code used to generate the results and figures in this paper is available on GitHub at \url{https://github.com/sepehrbayat/phronesis-index-nmi}. The data is available from the corresponding author upon reasonable request.
