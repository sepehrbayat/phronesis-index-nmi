...
As discussed in Sec.~\ref{sec:checklist}, defining stalks and restriction maps requires deep domain knowledge. This is a significant barrier to adoption:
\begin{itemize}
    \item \textbf{Non-expert users} may struggle to translate their system into a sheaf, limiting the method's accessibility.
    \item \textbf{Incorrect sheaf design} (e.g., choosing inappropriate restriction maps) can lead to misleading $\Phi$ values. We provide guidelines (Sec.~\ref{sec:checklist}, Appendix~\ref{app:sheaf_guide}), but no formal verification exists to check if a sheaf correctly captures the intended consistency semantics.
\end{itemize}
