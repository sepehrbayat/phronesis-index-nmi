...
\begin{itemize}
    \item \textbf{Continuous control:} We tested only discrete actions (Logic Maze, grid-world with discretized actions). Continuous action spaces (e.g., robotic manipulation) may require different sheaf constructions.
...
While we compared Q-learning+STPGC to a cost-penalty baseline (analogous to constrained RL), we did not compare to simpler consistency checks:
\begin{itemize}
